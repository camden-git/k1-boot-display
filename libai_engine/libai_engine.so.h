...
typedef struct Results Results, *PResults;

struct Results { // PlaceHolder Structure
};

typedef dword _ai_mode;
...
typedef struct Mat Mat, *PMat;

struct Mat { // PlaceHolder Structure
};
...
char * ai_engine_read_write(char *imagePath,_ai_mode mode,Results *results,int *resultCount,char *outputPath);
